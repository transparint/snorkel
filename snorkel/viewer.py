...
SNORKEL_HOME = os.path.join("..", "..", os.path.dirname(os.path.realpath(__file__)))
...
class Viewer(widgets.DOMWidget):
    ...
    def render(self):
        ...
        self.cids = cids
        self.html = open(SNORKEL_HOME+'/viewer/viewer.html').read() % (self.height, ''.join(pages))
        display(Javascript(open(SNORKEL_HOME + '/viewer/viewer.js').read()))
